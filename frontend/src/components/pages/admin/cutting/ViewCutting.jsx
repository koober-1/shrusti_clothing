...
/**
 * Helper function to aggregate entries by date and product name
 * This prevents "duplicate" rows (multiple entries for the same product/date) on the receipt.
 */
const aggregateEntries = (entries) => {
    if (!entries || !Array.isArray(entries)) return [];

    const grouped = entries.reduce((acc, entry) => {
        const date = formatDate(entry.date || entry.created_at || entry.entry_date);
        const productName = renderSafeValue(entry.product_name || entry.productName || entry.product);
...
        
        // Use the rate from the first entry to calculate the final rate
...
        acc[key].gross_amount += grossAmount;
        acc[key].original_entries_count += 1;
        
        // Recalculate rate based on aggregated totals
        acc[key].rate_per_piece = acc[key].pieces > 0 ? (acc[key].gross_amount / acc[key].pieces) : 0;
...
    gross_amount: 9440.00,
    deducted_advance: 1000,
    paid_amount: 8440.00,
    payment_type: 'Cash',
    pending_advance_before: 2000, // Added for clarity
    pending_advance_after: 1000 // Added for clarity
  };

  const data = paymentData || receiptDetails || sampleData;
  const entries = operatorEntries || [];
  
  // ✅ FIX: Aggregate entries to prevent "duplicate" rows
  const aggregatedEntries = aggregateEntries(entries);
  
  // Totals from aggregated entries (should match API totals)
  const totalGrossFromEntries = aggregatedEntries.reduce((sum, entry) => sum + parseNumber(entry.gross_amount), 0);
  const totalPiecesFromEntries = aggregatedEntries.reduce((sum, entry) => sum + parseNumber(entry.pieces), 0);
  const totalWeightFromEntries = aggregatedEntries.reduce((sum, entry) => sum + parseNumber(entry.weight), 0);

  // deductionAmount is calculated as (Gross - Payable)
  const deductionAmount = parseNumber(data.deductAdvance || data.deducted_advance);
  const finalPayableAmount = parseNumber(data.paidAmount || data.paid_amount || (totalGrossFromEntries - deductionAmount));

  // Determine the correct label for the deduction row based on sign
  const advanceLabel = deductionAmount >= 0 ? 'Advance Deduction' : 'New Advance Given';
  
  // Get Pending Advance Before/After Payment
  const pendingAdvanceBefore = parseNumber(data.pending_advance_before || data.pendingAdvanceBefore || 0);
  const pendingAdvanceAfter = parseNumber(data.pending_advance_after || data.pendingAdvanceAfter || 0);
...
      fontWeight: "normal"
    },
    advanceInfoSection: { // Style for advance balance display
        marginTop: "10px",
        paddingTop: "10px",
...
        fontSize: "14px",
        marginBottom: "30px", // Added space below for flow
    },
    advanceRow: {
        textAlign: "right",
    },
...
  const formatAdvance = (amount) => {
    const absAmount = Math.abs(amount);
    const sign = amount < 0 ? 'Given' : 'Due';
    const color = amount >= 0 ? '#dc2626' : '#16a34a'; // Red for Due (operator owes), Green for Given (company owes)
    return {
      text: `${sign}: ₹${absAmount.toFixed(2)}`,
      color: color,
      rawAmount: amount,
      label: sign,
    };
...
          </thead>
          <tbody>
            {/* Using aggregatedEntries here */}
            {aggregatedEntries && aggregatedEntries.length > 0 ? (
              aggregatedEntries.map((entry, index) => {
                const pieces = parseNumber(entry.pieces);
                const grossAmount = parseNumber(entry.gross_amount);
                const ratePerPiece = parseNumber(entry.rate_per_piece);
                const weight = parseNumber(entry.weight); 

                return (
                  <tr key={entry.date + entry.product_name} title={entry.original_entries_count > 1 ? `Aggregated from ${entry.original_entries_count} entries.` : ''}>
                    <td style={styles.tableCell}>{index + 1}</td>
                    <td style={styles.tableCell}>
                      {entry.date}
                    </td>
                    <td style={styles.tableCell}>
                      {renderSafeValue(entry.product_name)}
                    </td>
                    <td style={styles.tableCell}>
...
      </div>
 ...
   {/* Advance Balance Info - Moved to a prominent spot below the work table */}
      <div style={styles.advanceInfoSection}>
        <div style={styles.advanceRow}>
          <strong>Pending Advance (Before Payment):</strong><br />
          <span style={{ color: formatAdvance(pendingAdvanceBefore).color, fontWeight: 'bold' }}>
            {formatAdvance(pendingAdvanceBefore).text}
          </span>
        </div>
        <div style={styles.advanceRow}>
          <strong>Pending Advance (After Payment):</strong><br />
          <span style={{ color: formatAdvance(pendingAdvanceAfter).color, fontWeight: 'bold' }}>
            {formatAdvance(pendingAdvanceAfter).text}
          </span>
        </div>
      </div>

      {/* Signature Section - CORRECTLY PLACED AFTER the summary/payment info */}
...
  const [pendingAdvance, setPendingAdvance] = useState(0); // Advance BEFORE payment
  const [deductAdvance, setDeductAdvance] = useState(0); // Advance DEDUCTED/GIVEN
  const [payableAmount, setPayableAmount] = useState(0); // Final Paid Amount
...
      setPaidReceipts([]);
      setMessage('Failed to fetch paid receipts.');
    } finally {
      setLoading(false);
    }
  };
...
    const pendingAdvancePrint = parseNumber(selectedOperator.pending_advance);
    const payableAmountPrint = parseNumber(selectedOperator.payable_amount);
    
    // Calculate Deduction based on Gross - Payable
    // Note: This is an *unpaid* view, so the payable amount here is the *Net Payable* if all advance is deducted.
...
    // Advance After: Advance Before - Calculated Deduction (Gross - Payable)
    const pendingAdvanceAfterPrint = pendingAdvancePrint - calculatedDeduction;

    // ✅ FIX: Aggregate entries for the print view table
    const aggregatedEntries = aggregateEntries(operatorEntries);
    
    // Helper function to format advance for HTML print
    const formatAdvanceHTML = (amount) => {
        const absAmount = Math.abs(amount);
        const sign = amount < 0 ? 'Given' : 'Due';
        const color = amount >= 0 ? '#dc2626' : '#16a34a'; 
        return `<span style="color: ${color}; font-weight: bold;">${sign}: ₹${absAmount.toFixed(2)}</span>`;
    };
    
...
                .company-text { font-size: 24px; font-weight: bold; text-transform: uppercase; line-height: 1.2; letterSpacing: 1px; }
                .wages-title { font-size: 48px; font-weight: bold; color: #2563eb; letterSpacing: 3px; margin-top: 10px; text-align: right;}
                .wages-subtitle { font-size: 14px; font-weight: normal; margin-top: 5px; color: #666; }
                
                .staff-info { margin-bottom: 10px; font-size: 16px; display: flex; justify-content: space-between; align-items: flex-start; }
                .staff-left { display: flex; flex-direction: column; gap: 8px; }
                .staff-right { font-size: 16px; font-weight: normal; text-align: right;}
                
                /* New Advance Info Section */
...
                .table-total-row td { font-weight: bold !important; }
...
                            return `
                                <tr title="${entry.original_entries_count > 1 ? `Aggregated from ${entry.original_entries_count} entries.` : ''}">
...
                            <td colspan="3" class="table-cell" style="text-align: right; font-size: 14px;">**TOTALS:**</td>
                            <td class="table-cell" style="font-size: 14px; color: #16a34a;">${totalPieces}</td>
                            <td class="table-cell" style="font-size: 14px; color: #16a34a;">${totalWeight.toFixed(2)}</td>
                            <td class="table-cell">-</td>
                            <td class="table-cell" style="font-size: 14px; color: #16a34a;">₹${totalGrossAmount.toFixed(2)}</td>
                        </tr>
                    </tbody>
                </table>

                <div class="advance-info-section" style="margin-bottom: 25px;">
                    <div class="advance-row">
                        <strong>Pending Advance (Before Payment):</strong><br />
                        ${formatAdvanceHTML(pendingAdvancePrint)}
                    </div>
                    <div class="advance-row">
                        <strong>Pending Advance (After Payment):</strong><br />
                        ${formatAdvanceHTML(pendingAdvanceAfterPrint)}
                    </div>
                </div>
...
                                <span>Total Payable (to be paid):</span>
                                <span>₹${payableAmountPrint.toFixed(2)}</span>
                            </div>
                        </div>

                        <div class="payment-method">
                            *This is an estimated payment for entries*
                        </div>
                    </div>
                </div>
...
      let pending = 0;
      let defaultDeductAdvance = 0;
      let calculatedPayable = gross;

      if (staffResponse.data.success) {
        pending = parseNumber(staffResponse.data.pendingBalance);
        setPendingAdvance(pending);
        // Automatically deduct up to the gross amount
        defaultDeductAdvance = Math.min(pending, gross);
        calculatedPayable = gross - defaultDeductAdvance;
...
    // A positive deductAdvance means it was deducted/repaid to company. (Reduce pending advance)
    // A negative deductAdvance means a new advance was given by the company. (Increase pending advance)
...
    const currentDeductAdvance = parseNumber(deductAdvance);
    const currentPendingAdvance = parseNumber(pendingAdvance);

    // ✅ FIX: Calculate pending advance AFTER this payment
    // If deductAdvance is positive (deduction), it reduces the pending advance.
    // If deductAdvance is negative (new advance given), it increases the pending advance (more owed to company, or less owed to operator).
    const pendingAdvanceAfterPayment = currentPendingAdvance - currentDeductAdvance; 
...
        pendingAdvance: currentPendingAdvance, // Pending advance *before* this payment
        deductAdvance: currentDeductAdvance, // Amount deducted/given
        payableAmount: parseNumber(payableAmount), // Final paid amount
...
      // Calculate totals for the receipt (using original entries as they contain total pieces/weight)
...
        deducted_advance: currentDeductAdvance,
        deductAdvance: currentDeductAdvance,
        paid_amount: parseNumber(payableAmount),
        paidAmount: parseNumber(payableAmount),
        payment_type: paymentType,
        paymentType: paymentType,
        // ✅ FIX: Sending the correct 'before' and 'after' advance values to the receipt component
        pending_advance_before: currentPendingAdvance, 
        pending_advance_after: pendingAdvanceAfterPayment, 
      };
...
      
      // Retrieve the pending advance before and after from the receipt data
      const pendingBefore = parseNumber(receiptData.pending_advance_before || 0);
      const deducted = parseNumber(receiptData.deducted_advance || 0);
      // The API may not return 'pending_advance_after', so we calculate it: Before - Deducted
      const pendingAfter = pendingBefore - deducted;

      const fullReceiptData = {
          ...receiptData,
          pending_advance_before: pendingBefore,
          pending_advance_after: pendingAfter,
      };

      setReceiptToView(fullReceiptData);
      setOperatorEntries(entriesData);
      setShowPaidReceiptModal(true);
    } catch (error) {
...
  // Group operators by name (This section is for displaying a SINGLE, AGGREGATED row per operator)
...
    // The API already returns the aggregate values for these fields.
    acc[name].total_pieces = pieces; 
    acc[name].total_weight = weight; 
...
    ...group,
    // Restore logic for visual flag if more than one entry contributed to this aggregate row
    isAggregated: group.entries.length > 1, 
    originalData: group.entries, 
...
                        ...tableRowStyles,
                        backgroundColor: operator.isAggregated ? '#fef3c7' : 'transparent'
                      }}>
                        <td style={tableCellStyles}>{index + 1}</td>
                        <td
                          style={{
                            ...tableCellStyles,
                            color: operator.isAggregated ? '#d97706' : '#2563eb',
                            fontWeight: 'bold',
                            cursor: 'pointer',
                            position: 'relative'
                          }}
...
                          title={operator.isAggregated ? `Aggregated from ${operator.entries.length} entries. Click to view.` : 'Click to view entries'}
                        >
                          {operator.operator_name}
                          {/* Restore visual tag for aggregated entries */}
                          {operator.isAggregated && (
                            <span style={{
...
                          fontWeight: 'bold'
                        }}>
                          {/* Display sign based on advance: positive (>=0) is a due, negative is an amount given/advance given */}
...
                          fontWeight: 'bold'
                        }}>
                          {payable >= 0 ? '₹' + payable.toFixed(2) : '-₹' + Math.abs(payable).toFixed(2)}
                        </td>
                        <td style={{ ...tableCellStyles, textAlign: 'center' }}>
                          <button
...
                <div>
                  <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '4px' }}>
                    Pending Advance (Before Payment)
                  </label>
                  <input
                    type="text"
                    // Display sign based on advance: positive (>=0) is a due, negative is an amount given/advance given
...
                    <span style={{ color: '#0071bc', fontWeight: 'bold', fontSize: '1.1rem' }}>
                        {parseNumber(selectedOperator.payable_amount) >= 0 ? '₹' + parseNumber(selectedOperator.payable_amount).toFixed(2) : '-₹' + Math.abs(parseNumber(selectedOperator.payable_amount)).toFixed(2)}
                    </span>
                </div>
            </div>
            {/* END: Summary Card */}
...
                </thead>
                <tbody>
                  {/* Using non-aggregated entries here for the detailed view inside modal */}
